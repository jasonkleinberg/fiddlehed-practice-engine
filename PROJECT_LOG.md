...
### 2026-07-15 — v1.7: repeats/voltas actually play + fingering staircase fix + Kerry Polka corrected (on disk, NOT pushed)

Jason's retest found three problems; all fixed this session.

**1. Repeats were ignored (the big one).** The parser read measures linearly — `‖: A :‖` played once, and 1st/2nd endings played back-to-back as if they were plain bars (heard on 15.06 Irish Washerwoman / Kerfunken / Ballydesmond). Fix: new `expandRepeats()` expands the measure list into playback order before parsing — forward/backward repeat barlines, `times` attr, and volta brackets (skip endings whose numbers don't match the pass). 49 of 68 tunes have repeats; 11 have voltas. All now expand sanely (verified headless, whole library): Washerwoman plays A+1st, A+2nd, B+1st, B+2nd. Consequences handled: `buildNoteMap` rewritten to walk the graphic sheet and map each drawn note to EVERY pass (same SVG note lights on each repeat; click-to-seek prefers the pass inside the active loop section); section shade now maps played beats → notated measures via the new `playInstances`. **Discovery: `fisher-s-hornpipe.musicxml` has malformed volta marks** (2nd-ending start stamped on the same measure whose right barline is the backward repeat). Handled generically in the expander — trust the repeat, shift the ending start one bar right. Note contents confirm the intent (1st ending ends B4→C5 pickup-back, 2nd ends C5→D5 into the B part).

**2. Fingering staircase (Ballydesmond screenshot).** Some Sibelius exports wrote fiddle-tab fingerings as free-floating `<words>` directions; OSMD stacks each higher to dodge the last → diagonal chaos climbing off the page, plus an "Arranged for awesome fiddle students…" blurb floating mid-score. Affected: ballydesmond-lower (65), kerry-polka (58), peacock-rag (53), cripple-creek (35), twinkle (21), i'll-fly-away (14), drunken-sailor (11, tab-crammed), lucy-farr's (7, "AL1" style). Fix in the app, display-only: `preprocessForDisplay()` converts each words-fingering to a real `<fingering>` on the following note (OSMD anchors it to the notehead) and drops the blurb. Token pattern `[GDAE]?[LH]?[0-4]` covers "2", "A0", "G1", "L2", "H3", "AL1". Gotcha caught by the headless test: `XMLSerializer` drops the `<?xml` declaration and OSMD refuses the string — re-prepended.

**3. Kerry Polka was Spootiskerry.** Confirmed by melody diff (old kerry-polka.mxl = the D-E-G Spootiskerry figure; real Kerry Polka opens B-D-E-D). Jason supplied the correct file → now `music/kerry-polka.musicxml` (4.02, 16 bars AABB, chords in). **Index entries 16.07 (Backup Chords) and 17.04 (Double Stops) REMOVED** — their files were built from the wrong tune. Re-export both from the real Kerry Polka when convenient (sources still in `Practice Engine - Already XML/` as .mxl, but they're the wrong tune — need fresh Sibelius work). Library is 68 playable tunes.

Verified headless (jsdom + real OSMD 1.9.0): 68/68 parse, note-map entries exactly match audio note counts incl. repeats (ballydesmond 130/130, washerwoman 190/190), zero unmapped SVG notes, fingerings render anchored. 3 files (kesh ×2, oh-susannah-double-stops) emit multi-voice `backup` — events schedule fine, pre-existing, no action.

**Next:** Jason hard-refresh (v1.7 badge), listen for correct repeats, eyeball Ballydesmond + Kerry Polka + Peacock Rag scores. Then the deferred cleanup: consolidate the XML folders (`music/` vs `Practice Engine - Already XML/` vs `Practice Engine - XML output/` + `_needs-review/`, `_bonus-not-core/`) — one canonical folder, archive the rest.

### 2026-07-15 (cont.) — v1.8: fingerings stripped from the app + pickup-repeat file fixes (on disk, NOT pushed)

**Jason's call: NO fingerings in the play-along at all** — the app doubles as sheet-music reading practice; fingering/tab views belong on the lesson pages. So v1.7's convert-to-anchored-fingerings became v1.8's strip-everything: `preprocessForDisplay()` now REMOVES all fingering `<words>` directions (token pattern `[GDAE]?[LH]?[0-4]`, incl. tab-crammed multi-token), removes any native `<technical><fingering>` elements, and drops the "Arranged for awesome fiddle students by FiddleHed" blurb (now matches BOTH halves of the split text — v1.7 only caught the first line, which is why Twinkle still showed "Arranged for awesome fiddle D3"). `drawFingerings: false` as belt-and-braces. Display-only: the source XMLs keep their fingerings for lesson-page use.
...
### 2026-07-15 (cont.) — v1.11: loop sections built from repeat structure (on disk, NOT pushed)

Jason's catch: Britches Full of Stitches subdivided into 3-bar chunks instead of 2. Root cause: `buildSections()` halved the PLAYED timeline, which breaks when parts repeat unevenly — Britches' A repeats (16 played bars) but its B has a forward repeat and no backward one (plays once, 8 bars), so "A" grabbed a part and a half and quarters came out 24/2/4 = 3 bars.

Fix: `expandRepeats()` now also emits the SECTION structure (each closed repeat region = one musical part, with per-pass start positions + volta flag); `buildSections()` builds buttons from those parts. Span rules: no voltas → ONE pass (looping A once ≡ looping A-A, chunks stay 2 bars); voltas → full span, preserving the behavior Jason explicitly praised (Cripple Creek A4 = bar 3 + 2nd ending — verified intact). Stub "parts" under 2 bars (written-out ending bars trailing a repeat: coleraine, done-gone, give-the-fiddler-a-dram) are skipped so they don't steal letters. Bonus: 3-part tunes now get real C buttons (kesh-jig, the-butterfly, hava-negilah).

Verified across all 68 (headless, real buildSections): Britches A:8/B:8 with 2-bar quarters; zero range violations. Known data-driven quirks for the proofread: peacock A=7.25 bars (¾-bar pickup + 1-beat closing bar inside the repeat → no A quarters), fisher/swallowtail 15-bar parts (uneven 2-bar/1-bar endings → no quarters), wildwood 9-bar halves via fallback. Brief updated: new "check the Loop buttons" step + badge requirement bumped to v1.11. **Britches' B-part forward-repeat-without-backward may itself be a notation error — one for Jocelyn's list.**

### 2026-07-19 — v1.12: hide-sheet-music toggle (memorization practice)

Jason's request: a toggle to hide the sheet music so students can practice tunes (and chords) from memory. Implementation: "Hide sheet music" button at the top of the score box — hiding is pure CSS (`score-hidden` class on the wrap; the SVG stays in the DOM, paint loop keeps running harmlessly). Gotcha handled: OSMD can't render into a `display:none` container, so if the tune changes (or the window resizes) while hidden, `renderScore` stashes the XML and sets `scoreDirty`; the reveal click re-renders. Small italic "Playing from memory. You've got this." shows in place of the score. APP_VERSION → **1.12** (+ `?v=1.12` on the script tag).

### 2026-08-26 — v1.19: QA-sheet sweep (engine + 13 tune files), from Jason's beta spreadsheet

Full triage: `BETA_FIX_PLAN.md`. Engine (playalong.js/html): **load-time XML preprocessor** — consecutive duplicate `<harmony>` collapsed for BOTH audio (organ stops re-attacking every bar; suspected chunk of the "pulsing" complaint) and display (chord symbol only on change, reset at forward repeats), plus `<metronome>` directions stripped from the score. **Layout**: volume sliders moved above the score, transport made sticky (the WP iframe is 400px — everything scrolls), and autoScroll now yields to the user (manual scroll within 1.5s wins; note fully off-screen after a manual scroll = stay put) — fixes "I have to pause to scroll down." **Kick grid**: pattern now starts at `bodyStartBeats`, not transport 0 — cotton-eyed-joe/coleraine kicks were a pickup-length off every bar, forever; kick is now silent through any anacrusis. **Parser**: measure end snaps to the longest voice (`measureMax`) — kesh-jig__17.09's partial voice-2 after `<backup>` ended bar 11 256 ticks short, crashing bar 12 in early. **Melody lag** (>100 BPM): `__leadTempo` 0.05→0.08, lead cap 0.25→0.32. Kick default volume 100%→30%; "red note" hint removed.

Tune XML fixes (chords): fire-on-the-mountain (A part opens on A), old-joe-clark-variation-1 (m5 A), harvest-home (m8/10/11/12/14/16 per Jason's chart), lucy-farr-s (m8 |F Bb|), house-of-the-rising-sun (m7 |Am E|), auld-lang-syne (m7 |G C|), soldier-s-joy (m7+m15 |D A|), whiskey-before-breakfast (m14 |G D|), saint-anne-s-reel (m8 |G A|). Repeats: saint-anne-s-reel + lucy-farr-s B-part backward repeats added (were playing once). Endings rebuilt with proper volta brackets: **done-gone** and **give-the-fiddler-a-dram** (both had bracket-less double backward repeats) and **fisher-s-hornpipe** (brackets were shifted one bar left — m8 ends in the A-pickup, m9 in the B-lead-in, melody-verified). NOTE: v1.11 treated these three tunes' broken ending bars as "stub parts" — Loop buttons on them need a recheck. girl-i-left-behind-me m7 D5→D4 (open D). Removed from index: bile-em-cabbage-down__16.03 (chord-backup lesson, superseded by future chord viewer). coleraine index key → "Dm/G".

NOT done (deferred): B1 wrong-tune re-exports (Jason, in Sibelius: fiddle-fiddle 1.19, bile-var-1 6.04, banshee 10.07, kesh-jig single-note 12.05, kerry-polka DS 17.04) · southwind + coleraine pickup restructures (Sibelius) · bucket C pickup-in-repeat rewrites · pulsing re-listen after dedup ships.

### 2026-08-26 (cont.) — v1.20: self-hosted trimmed violin samples (the melody/kick lag, attacked at the root)

Jason's key observation: chords+kick = tight, chords+melody = tight, melody+kick = loose at speed. Both tight pairs involve synths; the loose pair is the SAMPLER — so the lag is entirely the violin samples' attack. Downloaded all 15 CDN mp3s and measured them offline: 20-40ms leading silence plus a slow bow swell, and the v1.x VIOLIN_ONSET table was measured as 30%-of-GLOBAL-PEAK on 11-17s crescendo samples — the peak lands seconds in, so the reference was wrong (E6 tabled at 157ms; true perceived onset after trim: 46ms). Fix: trimmed each sample (cut silence + ramp below 15% of early-sustain level, 8ms fade-in, capped 4.5s w/ 60ms fade-out), self-hosted as `samples/violin/*.wav` (~5.7MB, one-time load, kills the jsdelivr runtime dependency), re-measured residual onsets from the FINAL files (t50 of early-sustain, from file start) → new VIOLIN_ONSET (now 9-78ms, was 29-157ms; G4 is the straggler at 78ms — right in melody range, likely most of what Jason heard). Compensation knobs dialed back since the root cause is gone: `__leadBias` 0.05→0.01, `__leadTempo` 0.08→0.02 (both still console-tunable). APP_VERSION → **1.20**. Ear-verify at 160 BPM on an eighth-note reel; if timbre lost too much bow softness, the trim threshold (15%) can come down.

### 2026-08-26 (cont.) — v1.21: measured Jason's recordings, then attack-normalized sample set (A/B-switchable)

Jason recorded two takes per the protocol (melody+kick, organ muted). Band-split analysis (kick <140Hz, melody >350Hz, onset detection on both): **160 BPM: median offset +3ms — systematic lag is GONE after v1.20.** 80 BPM: median −9ms (hair early). What remains is per-note JITTER (SD ~25ms both takes) — each sample's attack SHAPE differs, so one lead number per sample can't nail every note. Fix: **attack normalization** — gain-shaped each trimmed sample so its envelope rises to sustain level over a uniform 25ms ramp (gain capped +12dB, blended out by 200ms). Result: all 15 onsets land 11-13ms, sd 0.7ms (was 9-78ms). Per Jason ("don't throw the baby out with the bathwater — don't lose v1.20"), the v1.20 set stays untouched at `samples/violin/`; normalized set is NEW at `samples/violin-v2/`. playalong.js now has SAMPLE_SETS with per-set onset tables + lead knobs; **v2 is default, `&samples=v1` on the URL A/Bs the old set.** Risk to ear-check: normalized attacks are more percussive/martelé — if the bow swell is missed, either stay on v1 (one-line default flip) or lower the ramp. APP_VERSION → **1.21**.


### 2026-09-05 — v1.22: QA-sheet round 2 — 3 re-exported tunes wired, 4 pickup rewrites, "pickup inside the repeat" becomes a first-class engine convention (on disk, NOT pushed)

Jason re-checked the sheet after v1.19–1.21; most rows were already closed. What was left:

**Re-exports dropped in by Jason (wired):** `banshee.musicxml` → now THE Banshee (G, **4/4** reel, 16 bars AABB, chords) — index time signature fixed 6/8→4/4, title "The Banshee". `Fiddle, Fiddle, Little Star.musicxml` → renamed `fiddle-fiddle-little-star.musicxml` (12 bars, whole-tune repeat) — **file is in D (fifths=2, chords D/G/A); sheet + old index said A → index now D, flag for Jason.** `bile-em-cabbage-down-variation.musicxml` → renamed `bile-em-cabbage-down__6.04.musicxml` (8 bars, repeat). All three: 1 part, no fingering words, no metronome marks.

**Pickup rewrites (XML):** fisher-s-hornpipe — B 2nd ending's closing rest → B4-C#5 eighths (Jason's spec). girl-i-left-behind-me — new 1-beat pickup bar G5-F#5 before bar 1 + the same two notes replace the rest in the B 2nd ending; **the hidden duet part got a matching rest-only pickup bar** (without it OSMD dropped the last measure of part 1 from the note map — multi-part files must keep measure counts equal). southwind + coleraine — restructured to the Peacock Rag form `|: pickup | bars… | short closing bar :|` for BOTH parts: pickup bar implicit, forward repeat moved onto it, the old trailing pickup notes removed from the closing bar (southwind A pickup Bb, B pickup C in its own 1-beat bar; coleraine A pickup D, B pickup B in its own eighth-note bar; coleraine's broken double-backward-repeat m9 and written-out final m18 removed, stale "1st end/2nd end" text stripped). Each part is now exactly 8 whole bars per pass.

**Engine — third pickup convention (`pickupInRepeat`):** first measure short AND carrying a forward repeat. Then (a) the Full loop runs the whole timeline instead of the overlap trick (which was chopping the last `pickupBeats` of every pass — Peacock Rag had been losing 3 beats at the wrap since v1.7), stopping at the last `:|` so a written-out final bar after it (Peacock's closing D) is an ending, not part of the loop; (b) the A section starts ON its pickup instead of being clipped to the first downbeat, so A/B loop seamlessly and quarters appear (Peacock A was 7.25 bars → now 8, A1–A4 exist). Kick grid unchanged (still starts at `bodyStartBeats`; per-pass length is whole bars so it stays aligned).

**Index edits:** coleraine key **"Dm/G" → "Am/C"** (fifths 0/minor, melody+chords are A minor / C major — the sheet note was a paste from Lonesome Fiddle Blues; flag for Jason). Relative-minor labels per the sheet's "could be labeled": shady-grove Dm, cluck-old-hen Am, drunken-sailor Dm, foggy-dew Dm, lonesome-fiddle-blues Dm/G. angeline-the-baker title lost its "(A)" (key already shown after the title).

**Verified headless (real Chromium + OSMD 1.9.0, all 67 indexed tunes):** every tune loads at v1.22, note-map count == audio note count on all files that matched before (girl 162/162, fisher 240/240, coleraine 174/174, southwind 88/88, banshee 232/232), zero page errors. southwind/coleraine/peacock: loop = 96/96/128 beats, A and B = 8 bars, A1–B4 present. Screenshots eyeballed for coleraine, southwind, girl, fisher.

**Still open:** 12.05 Kesh Jig single-note re-export + 17.04 Kerry Polka double-stops (Jason/Sibelius) · A3 pulsing + v1.21 sample-set ear check · Bucket C (Danny Boy short bars, Hobart AABB, Oh Susannah DS one-voice, Mary one-part, Butterfly overlap, remaining pickup-in-repeat rewrites — Wildwood, Bill Cheatham, Saint Anne's, Foggy Dew, Dram, Soldier's Joy Int, Washerwoman, Rights of Man).

### 2026-09-05 (cont.) — v1.22 addendum: Kesh Jig re-exports wired + Kerry Polka double stops GENERATED from the basic file

Jason dropped `kesh-jig-single-note` (→ `kesh-jig.musicxml`, 12.05; 2 parts, duet hidden by the app) and `kesh-jig-adding-double-stops` (→ `kesh-jig__17.09.musicxml`). **The DS file's B part had a forward repeat at m9 but no backward repeat at m16 → B played once; added the `:|`.** Its bars 9–13 write the double stops as a second `<backup>` voice (the "two separate lines" style Jason disliked on Oh Susannah DS) — engine copes (measureMax snap), display is his call.

**Kerry Polka – Adding Double Stops (17.04) built by script, not Sibelius:** Jason's rule — A-string note → add open D under it; D-string note → add open G; rhythm identical. The basic file's own fingering `<words>` (A1, D0, D3…) name the string for every note, so the string was read from those (pitch fallback never needed). Each drone is a `<chord/>` note after the melody note (one voice, stems shared — the Kesh-style single-line look). 58 drones added: B4/A4/D5 → D4 (26), D4/E4/G4 → G3 (32). Index: `kerry-polka__17.04`, module 17, lesson URL from the QA sheet; index re-sorted by lesson id (68 tunes). Headless: 232/232 notes mapped, A/B/quarters present; screenshot eyeballed.
